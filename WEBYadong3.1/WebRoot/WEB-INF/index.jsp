...
<script>
	jQuery(function() {
								...
								$(".postBtn").click(function() {
									...
									if ("${user.name}") {
										if ($(".Input_text").val()) {
											...
											alert("buwei"+$(".Input_text").val());
											console.log("..."+$(".Input_text").val());
											console.log("..."+"${user.name}");
											console.log("..."+"${user.id}");
											console.log("..."+$(".postBtn :first-child").val());
										//	console.log(".1213.."+$("#rid1").val());
											 
											
											$.ajax({
												type : 'POST',
												url : "${pageContext.request.contextPath}/commentAction_save.action?randnum="+Math.floor(Math.random()*1000000),
												data : {username:"${user.name}",userId:"${user.id}",commentCon:$(".Input_text").val(),riddleId: $(".postBtn :first-child").val()},
												success : function(data) {
													//  alert(data.rows.length+"123");
													console.log("susess:"+data.rows);
													for (var i = 0; i < data.rows.length; i++) {
														var j = i + 1;
														$("#postList" + j).show();
														$("#title" + j).html("sda");
														$("#span" + j).html(data.rows[i].riddleSide);$("#span-" + j).html("hhhh");
													}
												}
		...
		//	})
			...
			alert("humorId="+humorId);
	alert($("#spanPlus_" + humorId).html());
    var spanPlus = $("#spanPlus_" + humorId).html();
	console.log("scope"+spanPlus);
    
    var isShowPinLun=$("#isshowpinlun_"+humorId).val();
    alert($("#isShowPinLun" + isShowPinLun));
...
<body>
				...
				<div class="next-site menu-list">
					<ul class="am-nav am-nav-pills am-topbar-nav">
						<c:if test="${user!=null}">
							...
							<li><a id="regedit" href="${pageContext.request.contextPath}/hmModelDatasetMpping_forWord.action">${user.name }你好</a></li>
						</c:if>
						<c:if test="${user==null}">
							<li><a id="loginId"
								href="${pageContext.request.contextPath}/hmModelDatasetMpping_doLogin.action">登陆</a></li>
							<li><a id="regedit" href="${pageContext.request.contextPath}/hmModelDatasetMpping_forWord.action">注册</a></li>
						</c:if>
					</ul>
				</div>
			</div>
   ...
   <div class="am-u-lg-5">
        ...
        <div class="hfbox">
            ...
            <div class="dingBox">
                <div class="rssTab" id="userTag_1204617">
                    
                    <input type="hidden" value="" id="listTags1204617" />
            ...
            <div class="hufuInput" id="divCommentContent_${list.riddleId }" style="display: none">
                <div class="textBox">
                    <textarea name="" cols="" rows="" class="Input_text" ></textarea>
                ...
                <div class="textBtn">
                    ...
                    <div class="zheZhaoBtn" id="zheZhaoBtn_1204617" onclick="ZheZhaoBtn()"><a href="###">选择图片上传</a></div>
                    <a onclick="postcommentinfo(${list.riddleId },false);pengfu_click(null,1204617,5610924);"  class="postBtn" >提交
            ...
            <div class="newSList" style="display: none" id="newList_${list.riddleId }">
				...
				<a class="jubao" onclick="report(3599371)" style="display: none;">举报</a></div></div></dd></dl>
                  ...
                  <!--   <p style="text-align: center; height: 20px; margin-top: 15px;" id="nopinglun_1204617">没有最新评论</p>-->
                    <div class="shouqiBtn" style="display: block" id="shouqiBtn_${list.riddleId }" onclick="shouqiBtn(1204617,this)"><span></span></div> 
                    <!--添加测试数据-->
                </div>
            </div>
            <!--评论页 结束-->
