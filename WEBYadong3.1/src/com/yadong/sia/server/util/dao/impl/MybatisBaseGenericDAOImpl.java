package com.yadong.sia.server.util.dao.impl;
...
import com.yadong.sia.server.util.bean.BeanMapUtil;
import com.yadong.sia.server.util.dao.IBaseGenericDAO;
import com.yadong.sia.server.util.exception.BaseDaoException;
import com.yadong.sia.server.util.page.GenericDefaultPage;
import com.yadong.sia.server.util.page.IGenericPage;
import com.yadong.sia.server.util.reflect.ReflectGeneric;
...
/*
	...
	private SqlSessionTemplate getSqlSessions(){
		try {
			String catalog = getSqlSession().getConnection().getCatalog();
			System.err.println("connection.........schema................"+catalog);
...
*/	
	...
	public IGenericPage<T> findPageBy(Map<String, Object> params, int pageNo, int pageSize, String sort, String dir){
		...
		if (StringUtils.isNotEmpty(sort) && StringUtils.isNotEmpty(dir)) {
			new PageBounds(pageNo,pageSize,Order.create(sort, dir));
		}else{
	    ...
	    return new GenericDefaultPage<T>(pageNo, pageSize, list, count);
	}
	...
	public IGenericPage<T> findPageBy(Map<String, Object> params, PageBounds pageBounds){
		...
		PageList pageList = (PageList) list;
		int count = pageList.getPaginator().getTotalCount();
	    return new GenericDefaultPage<T>(pageBounds.getPage(), pageBounds.getLimit(), list, count);
...
//			paramMap.put("dir", null);
		...
		List<T> lst = getSqlSession().selectList("","" );
...
//			paramMap.put("dir", null);
		...
		List<T> lst = getSqlSession().selectList(param,pa);
